...
def wait_until_processed(config: NamerConfig):
    ...
    """
    while (
        len(list(config.watch_dir.iterdir())) > 0
        or len(list(config.work_dir.iterdir())) > 0
    ):
        time.sleep(0.2)
...
class UnitTestAsTheDefaultExecution(unittest.TestCase):
    ...
    def test_handler_collisions_success(self, mock_poster, mock_response):
        ...
        with tempfile.TemporaryDirectory(prefix="test") as tmpdir:
            ...
            outputfile = (
                config.dest_dir
                / "EvilAngel - 2022-01-03 - Carmela Clutch Fabulous Anal 3-Way!"
    ...
    def test_handler_deeply_nested_success_no_dirname(self, mock_poster, mock_response):
        ...
        with tempfile.TemporaryDirectory(prefix="test") as tmpdir:
            ...
            watcher.start()
            targets = [
                new_ea(config.watch_dir / "deeper" /
                       "and_deeper", use_dir=False),
        ...
        with tempfile.TemporaryDirectory(prefix="test") as tmpdir:
            ...
            targets = [
                new_ea(
                    config.watch_dir
                    / "_UNPACK_stuff"
                    / "EvilAngel - Carmela Clutch Fabulous Anal 3-Way",
                    use_dir=True,
                ),
            ]
            time.sleep(2)
