...
from namer.types import NamerConfig, TargetFile, default_config
...
def gather_target_files_from_dir(dir_to_scan: Path, config: NamerConfig) -> Iterable[TargetFile]:
    ...
    if dir_to_scan is not None and dir_to_scan.is_dir() and dir_to_scan.exists():
        ...
        filterd: Iterable[TargetFile] = filter(lambda file: file is not None, mapped)  # type: ignore
        return filterd
    return []
...
def __exact_parse(target_movie_file: Path, target_dir: Path, parse_dir: bool, config: NamerConfig) -> TargetFile:
    ...
    targetfile.target_directory = target_dir
    targetfile.target_movie_file = target_movie_file
    targetfile.parsed_dir_name = parse_dir
    if target_movie_file is not None:
        name = target_movie_file.name if not parse_dir else (target_dir.name + target_movie_file.suffix)
        targetfile.parsed_file = parse_file_name(name, config.name_parser)
    return targetfile
...
def analyze(input: Path, config: NamerConfig) -> Optional[TargetFile]:
    """
    after finding target directory and target movie from input, returns file name descriptors.
    """
    target_dir = input if input.is_dir() else input.parent
    target_movie = input if not input.is_dir() else find_target_file(target_dir, config)
    parse_dir = input.is_dir() and config.prefer_dir_name_if_available
    target_file = __exact_parse(target_movie, target_dir, parse_dir, config)
    target_file.input_file = input
    output = target_file if is_interesting_movie(target_file.target_movie_file, config) else None
    return output


def attempt_analyze(input: Optional[Path], config: NamerConfig) -> Optional[TargetFile]:
    """
    Attempts to parse an input path after determining target movies/directories
    """
    return None if input is None else analyze(input, config)
