...
class Mutagen:
    ...
    def get_mp4_if_possible(self, mp4: Path) -> MP4:
        """
        ...
        try:
            ...
            self.__ffmpeg.attempt_fix_corrupt(mp4)
            video = MP4(mp4)
        return video

    @logger.catch
    def update_mp4_file(self, mp4: Path, looked_up: LookedUpFileInfo, poster: Optional[Path], config: NamerConfig):
        ...
        success = self.__ffmpeg.update_audio_stream_if_needed(mp4, config.language)
        if not success:
            logger.info("Could not process audio or copy {}", mp4)
        logger.info("Updating atom tags on: {}", mp4)
        if mp4 is not None and mp4.exists():
            video: MP4 = self.get_mp4_if_possible(mp4)
            video.clear()
            self.set_single_if_not_none(video, "\xa9nam", looked_up.name)
            video["\xa9day"] = [looked_up.date + "T09:00:00Z"] if looked_up.date is not None else []
            if config.enable_metadataapi_genres:
                self.set_array_if_not_none(video, "\xa9gen", looked_up.tags)
            else:
                self.set_array_if_not_none(video, "keyw", looked_up.tags)
                self.set_single_if_not_none(video, "\xa9gen", config.default_genre)
            self.set_single_if_not_none(video, "tvnn", looked_up.site)
            self.set_single_if_not_none(video, "\xa9alb", looked_up.site)
            video["stik"] = [9]  # Movie
            resolution = self.resolution_to_hdv_setting(self.__ffmpeg.get_resolution(mp4))
            self.set_single_if_not_none(video, "hdvd", resolution)
            self.set_single_if_not_none(video, "ldes", looked_up.description)
            self.set_single_if_not_none(video, "\xa9cmt", looked_up.source_url)
