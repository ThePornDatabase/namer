...
class MovieXML:
    ...
    def __init__(self, config: NamerConfig, file_utils: FileUtils):
    ...
    def write_movie_xml_file(self, info: LookedUpFileInfo, trailer: Optional[Path] = None, poster: Optional[Path] = None, background: Optional[Path] = None) -> str:
        """
        ...
        if self.__config.enable_metadataapi_genres:
            for tag in info.tags:
                etree.SubElement(root, "genre", attrib=None, nsmap=None).text = tag
        else:
            for tag in info.tags:
                etree.SubElement(root, "tag", attrib=None, nsmap=None).text = tag
            etree.SubElement(root, "genre", attrib=None, nsmap=None).text = self.__config.default_genre
    ...
    def write_nfo(self, video_file: Path, new_metadata: LookedUpFileInfo, trailer: Optional[Path], poster: Optional[Path], background: Optional[Path]):
        """
        Writes an .nfo to the correct place for a video file.
        """
        if video_file is not None and new_metadata is not None and self.__config.write_nfo is True:
            target = video_file.parent / (video_file.stem + ".nfo")
            with open(target, "wt", encoding="utf-8") as nfo_file:
                towrite = self.write_movie_xml_file(new_metadata, trailer, poster, background)
                nfo_file.write(towrite)
            self.__file_utils.set_permissions(target)
