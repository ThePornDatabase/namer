...
class FileNameParser:
    ...
    def parse_file_name(self, filename: str, regex_config: str = DEFAULT_REGEX_TOKENS) -> FileNameParts:
        """
        ...
        """
        regex = self.parser_config_to_regex(regex_config)
        ...
        if match:
            ...
            if match.groupdict().get("name") is not None:
                file_name_parts.name = self.name_cleaner(match.group("name"))
