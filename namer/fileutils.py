...
class FileUtils:
    ...
    __file_name_parser: FileNameParser

    def __init__(self, config: NamerConfig, file_name_parser: FileNameParser):
        self.__config = config
        self.__file_name_parser = file_name_parser

    def move_command_files(self, target: Optional[Command], new_target: Path) -> Optional[Command]:
        working_dir = None
        working_file = None
        ...
        if target.input_file == target.target_directory and target.target_directory is not None:
            ...
            output = self.make_command(working_dir)
        else:
            ...
            output = self.make_command(working_file)
    ...
    def write_log_file(self, movie_file: Optional[Path], match_attempts: Optional[List[ComparisonResult]]) -> Optional[Path]:
        """
        ...
        if movie_file is not None:
            ...
            self.set_permissions(log_name)
        return log_name

    def _set_perms(self, target: Path):
        file_perm: Optional[int] = (None if self.__config.set_file_permissions is None else int(str(self.__config.set_file_permissions), 8))
        dir_perm: Optional[int] = (None if self.__config.set_dir_permissions is None else int(str(self.__config.set_dir_permissions), 8))
        if self.__config.set_gid is not None:
            os.lchown(target, uid=-1, gid=self.__config.set_gid)
        if self.__config.set_uid is not None:
            os.lchown(target, uid=self.__config.set_uid, gid=-1)
    ...
    def set_permissions(self, file: Optional[Path]):
        """
        ...
        """
        if system() != "Windows" and file is not None and file.exists() and self.__config.update_permissions_ownership is True:
            self._set_perms(file)
            if file.is_dir():
                for target in file.rglob("*.*"):
                    self._set_perms(target)

    def move_to_final_location(self, command: Command, new_metadata: LookedUpFileInfo) -> Command:
        """
        ...
        if command.target_directory and not command.config.del_other_files and containing_dir is not None:
            ...
            self.set_permissions(target_dir / relative_path.parts[0])
        else:
            self.set_permissions(movie_name)
        ...
        if command.target_directory is not None and not self.subpath_or_equal(output.target_directory, command.target_directory):
    ...
    def is_interesting_movie(self, path: Optional[Path]) -> bool:
        if path is None:
            return False
        exists = path.exists()
        suffix = path.suffix.lower()[1:] in self.__config.target_extensions
        size = path.stat().st_size / (1024 * 1024) >= self.__config.min_file_size
        return exists and size and suffix

    def gather_target_files_from_dir(self, dir_to_scan: Path) -> Iterable[Command]:
        """
        ...
        if dir_to_scan is not None and dir_to_scan.is_dir() and dir_to_scan.exists():
            ...
            mapped: Iterable = map(lambda file: self.make_command((dir_to_scan / file)), dir_to_scan.iterdir())
            filtered: Iterable[Command] = filter(lambda file: file is not None, mapped)  # type: ignore
            return filtered
        return []

    def __exact_command(self, target_movie_file: Path, target_dir: Optional[Path]) -> Command:
        """
        ...
        command.parsed_dir_name = target_dir is not None and self.__config.prefer_dir_name_if_available
        command.config = self.__config
        name = target_movie_file.name
        parsed_dir_name = False
        if target_dir is not None and self.__config.prefer_dir_name_if_available:
            name = target_dir.name + target_movie_file.suffix
            parsed_dir_name = True
        command.parsed_file = self.__file_name_parser.parse_file_name(name, self.__config.name_parser)
        command.parsed_dir_name = parsed_dir_name
        return command

    def find_target_file(self, root_dir: Path) -> Optional[Path]:
        """
        ...
        if len(list_of_files) > 0:
            for target_ext in self.__config.target_extensions:
    ...
    def make_command(self, input_file: Path, nfo: bool = False, inplace: bool = False, uuid: Optional[str] = None) -> Optional[Command]:
        """
        ...
        target_movie = input_file if not input_file.is_dir() else self.find_target_file(input_file)
        if target_movie is None:
            return None
        target_file = self.__exact_command(target_movie, target_dir)
        target_file.input_file = input_file
        target_file.tpdb_id = uuid
        target_file.write_from_nfos = nfo
        target_file.inplace = inplace
        output = target_file if self.is_interesting_movie(target_file.target_movie_file) else None
        return output

    def make_command_relative_to(self, input_dir: Path, relative_to: Path, nfo: bool = False, inplace: bool = False, uuid: Optional[str] = None) -> Optional[Command]:
        """
        Ensure we are going to handle the directory relative to another directory, rather than just the file
        specified
        """
        if self.subpath_or_equal(input_dir, relative_to):
            relative_path = input_dir.absolute().relative_to(relative_to.absolute())
            if relative_path is not None:
                target_file = relative_to / relative_path.parts[0]
                return self.make_command(target_file, nfo, inplace, uuid)
        return None
    ...
    def main(self, arg_list: List[str]):
        ...
        target_file = self.make_command(target)
...
if __name__ == "__main__":
    ...
    file_utils.main(arg_list=sys.argv[1:])
