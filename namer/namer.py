...
def process_file(command: Command) -> Optional[Command]:
    ...
    if command.target_movie_file is not None:
        ...
        #    todo use phash
            ...
            phash = vph.get_hashes(command.target_movie_file, command.config.max_ffmpeg_workers, command.config.use_gpu) if command.config.search_phash else None
        ...
        if new_metadata is not None:
            if command.config.manual_mode and command.is_auto:
                ...
                if command.config.send_phash:
                    phash = vph.get_hashes(command.target_movie_file, command.config.max_ffmpeg_workers, command.config.use_gpu) if not phash else phash
                    if phash:
